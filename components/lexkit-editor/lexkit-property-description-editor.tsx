        ...
        <>
          ...
          {hasExtension("blockFormat") && (
            <>
              ...
              {hasExtension("code") && (
                <button type="button" onClick={() => commands.toggleCodeBlock()} className={`lexkit-toolbar-button ${activeStates.isInCodeBlock ? "active" : ""}`} title="Code Block"><Terminal size={14} /></button>
              )}
              <div className="w-px h-6 bg-border mx-1" />
            </>
          )}
          {hasExtension("list") && (
            <>
              <button type="button" onClick={() => commands.toggleUnorderedList()} className={`lexkit-toolbar-button ${activeStates.unorderedList ? "active" : ""}`} title="Bullet List">
                <List size={14} />
              </button>
              <button type="button" onClick={() => commands.toggleOrderedList()} className={`lexkit-toolbar-button ${activeStates.orderedList ? "active" : ""}`} title="Numbered List">
                <ListOrdered size={14} />
              </button>
            </>
          )}
    ...
    <>
      <div className="lexkit-toolbar">
        ...
        {hasExtension("list") && (
          <div className="lexkit-toolbar-section">
            ...
            {(activeStates.unorderedList || activeStates.orderedList) && (
              <>
                <button type="button" onClick={() => commands.indentList()} className="lexkit-toolbar-button" title="Indent List"><Indent size={14} /></button>
                <button type="button" onClick={() => commands.outdentList()} className="lexkit-toolbar-button" title="Outdent List"><Outdent size={14} /></button>
              </>
            )}
          </div>
        )}
        ...
        {hasExtension("image") && (
          <div className="lexkit-toolbar-section">
            ...
            {activeStates.imageSelected && (
              <Dropdown
                trigger={<button type="button" className="lexkit-toolbar-button" title="Align Image"><AlignCenter size={16} /></button>}
                isOpen={showAlignDropdown}
                onOpenChange={setShowAlignDropdown}
              >
                <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.setAlignment("left"); setShowAlignDropdown(false); }}><AlignLeft size={16} /> Align Left</button>
                <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.setAlignment("center"); setShowAlignDropdown(false); }}><AlignCenter size={16} /> Align Center</button>
                <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.setAlignment("right"); setShowAlignDropdown(false); }}><AlignRight size={16} /> Align Right</button>
                <button type="button" className="lexkit-dropdown-item" onClick={() => { handlers.setCaption(); setShowAlignDropdown(false); }}><Type size={16} /> Set Caption</button>
              </Dropdown>
            )}
        ...
        {hasExtension("history") && (
          <div className="lexkit-toolbar-section">
            <button type="button" onClick={() => commands.undo()} disabled={!activeStates.canUndo} className="lexkit-toolbar-button" title="Undo (Ctrl+Z)"><Undo size={16} /></button>
            <button type="button" onClick={() => commands.redo()} disabled={!activeStates.canRedo} className="lexkit-toolbar-button" title="Redo (Ctrl+Y)"><Redo size={16} /></button>
          </div>
        )}

        {/* Command Palette */}
        <div className="lexkit-toolbar-section">
          <button type="button" onClick={onCommandPaletteOpen} className="lexkit-toolbar-button" title="Command Palette (Ctrl+K)"><Command size={16} /></button>
        </div>

        {/* Theme Toggle */}
        <div className="lexkit-toolbar-section">
          <button type="button" onClick={toggleTheme} className="lexkit-toolbar-button" title={isDark ? "Light Mode" : "Dark Mode"}>
      ...
      <Dialog isOpen={showTableDialog} onClose={() => setShowTableDialog(false)} title="Insert Table">
        <div className="lexkit-table-dialog">
          ...
          <div className="lexkit-dialog-actions">
            <button type="button" onClick={() => setShowTableDialog(false)} className="lexkit-button-secondary">Cancel</button>
            <button type="button" onClick={() => { commands.insertTable(tableConfig); setShowTableDialog(false); }} className="lexkit-button-primary">Insert Table</button>
          </div>
        </div>
      </Dialog>
    </>
...
function EditorContent({
  ...
  useEffect(() => {
    ...
    const unregisterContentListener = editor.registerUpdateListener(({ editorState }) => {
      if (onChange && readyRef.current) {
        editor.read(() => {
  ...
  }, [editor, commands, onReady, methods, onChange]);

  // Handle value prop changes - load content into editor when value changes
  useEffect(() => {
    if (!editor || !commands || !value) return;
    
    // Only load content if it's different from what's currently in the editor
    editor.read(() => {
      const currentContent = commandsRef.current.exportToHTML();
...
export const DefaultTemplate = forwardRef<DefaultTemplateRef, DefaultTemplateProps>(({ className, onReady, value, onChange, placeholder, error }, ref) => {
...
// Export as LexkitPropertyDescriptionEditor for backward compatibility
export const LexkitPropertyDescriptionEditor = DefaultTemplate;
